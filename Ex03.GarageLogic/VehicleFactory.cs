...
namespace Ex03.GarageLogic
{
    ...
    class VehicleFactory
    {
        ...
        internal static Vehicle CreateVehicle(Dictionary<string, string> i_FieldsToFill, string i_VehicleType, string i_LicenseNumber)
        {
            eVehicleTypeSupported vehicleTypeSupported = ParseStringToVehicleTypeSupportedEnum(i_VehicleType);
            ...
            switch (vehicleTypeSupported)
            {
                case eVehicleTypeSupported.Car:
                    ...
                    vehicle = CreateCar(carEngineType, carColor, numberOfDoors, vehicleModel, i_LicenseNumber, float.Parse(energyLevel), wheelManufacturer, float.Parse(airPreasure));
                    break;
                ...
                case eVehicleTypeSupported.Motorcycle:
                    ...
                    vehicle = CreateMotorCycle(motorCycleEngineType, licenseType, float.Parse(engineCC), vehicleModel, i_LicenseNumber, float.Parse(energyLevel), wheelManufacturer, float.Parse(airPreasure));
                    break;
                ...
                case eVehicleTypeSupported.Truck:
                    ...
                    vehicle = CreateTruck(bool.Parse(isToxic), float.Parse(maxWeight), vehicleModel, i_LicenseNumber, float.Parse(energyLevel), wheelManufacturer, float.Parse(airPreasure));
                    break;
            }
            return vehicle;
        ...
        {
            if (i_MaxFuelTank < i_CurrentFuelTank)
            {
                throw new ValueOutOfRangeException("Energy amount", 0, i_MaxFuelTank);
            }
        ...
        {
            if (i_MaxEnergyLevel < i_CurrentBatteryLevel)
            {
                throw new ValueOutOfRangeException("Energy amount", 0, i_MaxEnergyLevel);
            }
